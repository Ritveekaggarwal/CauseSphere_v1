    ...
    try {
      const res = await fetch("http://localhost:5000/api/campaigns", {
        ...
        body: JSON.stringify({
          ...
          category: form.category.toLowerCase(), // 🔥 important
          endDate: form.endDate,
          email: form.email,
          phone: form.phone,
          payoutMethod: form.payoutMethod,
          upiId: form.upiId,
          idDoc: form.idDoc?.name || "",
        }),
      });
            ...
            <div className="space-y-3">
              {["Campaign info", "Payout", "Verify"].map((label, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className={`w-6 h-6 flex items-center justify-center border text-xs ${step > i + 1
                ...
                <div className="grid grid-cols-2 gap-4">
                  {["upi", "bank"].map((m) => (
                    <button
                      ...
                      className={`p-5 border transition ${form.payoutMethod === m
